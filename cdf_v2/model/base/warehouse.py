...
class Warehouse(BaseModel):
    ...
    classification_algorithm = ForeignKeyField(StaticData)                                                   # 分类方法
    ...
    current_k_image = ForeignKeyField(Image, null=True)  # k曲线图片
    first_classification_number = IntegerField(null=True)   # 第一类数量
    last_classification_number = IntegerField(null=True)  # 最后一类数量
    a_classification_percent = DecimalField(max_digits=24, decimal_places=10, null=True, auto_round=True)  # A类占比(%)
    b_classification_percent = DecimalField(max_digits=24, decimal_places=10, null=True, auto_round=True)  # B类占比(%)
