		...
		if rule1 and rule2:
			...
			for i in range(1,self.max_log):
				...
				if rule_1 and rule_2 and rule_3:
					print('[INFO]:The best log n is:{0}'.format(i))
					break
		return ts
	...
	def __ts_differencing(self): # 计算时间序列的差分d值
		...
		while True:
			...
			if not (rule_1 and rule_2) and self.d < self.max_diff_time:
				self.d = self.d + 1
				self.ts_diff = self.ts_diff.diff(1) #进行d阶差分
...
class ts_WMA:
	...
	def fit(self):
		if len(self.ts_train)==1:
			...
			min_error=inf
			y_train=list(self.ts_train) #转化为list
			end_window=min(len(self.ts_train)-1,self.max_window)
			for window in range(self.min_window,end_window+1):
...
if __name__=='__main__':
	...
	# print(type(y_fit))
	ts=Series([2.0, 3.0, 0.0, 0.0, 0.0, 3.0, 0.0])
	print(ts)
	model=ts_WMA(ts,evaluation_methods['RMSE'])
	model.fit()
	y_fit=model.get_fittedvalues()
	y_pre=model.predict(4)
