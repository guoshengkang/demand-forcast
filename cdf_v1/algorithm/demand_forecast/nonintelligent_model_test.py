...
def nonintelligent_model(input_data_df=None,config_data_df=None,date_start=None,date_end=None,time2market=None,forcast_model='GBRT',
	...
	for id in obj_ids: 
		...
		if forcast_model in regression_methods:
			...
			y_real,y_fit,y_predict,error_fit=integrated_deeplearning_model(dp_parameters=dp_parameters,df=df_tmp,forcast_model=forcast_model,evaluation=evaluation,length_merge=length_merge,period=period)
			...
			# y_real,y_fit,y_predict,error_fit=integrated_deeplearning_model(dp_parameters=best_dp_parameters,df=df_tmp,forcast_model=forcast_model,evaluation=evaluation,length_merge=length_merge,period=period)
			# print('&%$*'*20)
		...
		plt.xlabel('days')
		plt.title("id:%s model:%s\nfitted_MAPE:%.2f predicted_MAPE:%.2f integrated_MAPE:%.2f RMSE:%.2f"%(id,forcast_model,error_fit,error_pre,integrated_MAPE,RMSE_final))
		plt.legend() 
		fig.set_size_inches(16, 8)
		# plt.savefig(forcast_model+'_'+str(id)+'.png',dpi = 300,bbox_inches='tight',pad_inches=None) #,,bbox_inches='tight'
		plt.show()
        #####################################################################
...
if __name__=='__main__':
	...
	json_result=nonintelligent_model(input_data_df,config_data_df,date_start=date_start,date_end=date_end,time2market=time2market,
			forcast_model='ARIMA',evaluation='MAPE',length_merge=1,period=7)
