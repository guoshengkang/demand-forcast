...
def integrated_regression_model(df=None,forcast_model='GBRT',evaluation='MAPE',length_merge=2,period=5):
	'''
	回归的预测方法
	--input parameters--
	...
	for k,index in enumerate(X_df_test.index):
		...
		X_test=X_df_test.ix[index,:].values # 取一行的话,结果是向量
		X_test=np.array(X_test).reshape(1,len(X_test))
		# 模型训练
		if k==0:# 模型训练,只做一次
				# 模型初始化
				model=regression_methods[forcast_model]()
				# 模型训练
