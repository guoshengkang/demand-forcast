...
def integrated_timeseries_model(df=None,forcast_model='ARIMA',evaluation='MAPE',length_merge=1,period=7):
	'''
	时间序列的预测方法
	--input parameters--
	df:预测的输入数据--DataFrame(训练+预测)
	forcast_model:预测模型--str
	...
	# y的按星期平均值Series
	if forcast_model=='ARIMA' or forcast_model=='SARIMAX':
	...
	# (2) 模型训练及滚动预测
	model=timeseries_methods[forcast_model](ts_train=y_series_sub,error_fun=evaluation_methods[evaluation])
	model.fit()
	# print('$'*20)
