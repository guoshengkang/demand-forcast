...
def intelligent_model(input_data_df=None,config_data_df=None,date_start=None,date_end=None,time2market=None,forcast_model='ARIMA',
	...
	for index_no,id in enumerate(obj_ids): 
		...
		# CASE 2:有销量的数据>=10天,存在2次连续至少7天的销量均为0,则使用LSTM模型
			...
			forcast_model='LSTM'
		# CASE 3:数据量>=50,且满足2阶内差分可平稳
		elif con_ARIMA(y_series=y_train):
			forcast_model='ARIMA'
		# CASE 4:数据量>=50,且满足1阶差分+周期差分可平稳
		elif con_SARIMAX(y_series=y_train):
			forcast_model='SARIMAX'
		# CASE 5:默认的方法
		else:
			forcast_model='WMA'
