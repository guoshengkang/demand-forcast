...
from algorithm.demand_forecast.deeplearning_models import *
...
def integrated_deeplearning_model(
	isIntelligent=True,
	sku_id='',
	df=None,
	forecast_model='LSTM',
	...
	# 增加t-1,t-2,t-7的特征
	feature_lag={'quantity':[1,2,7],'sale_price':[1],'purchase_price':[1],'weekend':[1]}
	...
	for k,index in enumerate(X_df_test.index):
		...
		if k==0: # 特征选取,只做一次
			...
			# print('#'*10,final_features)
			# if forecast_model=='CNN':
			# 	final_features=features
			X_train=X_df_train.ix[:,final_features].values # 特征选择之后的X
		...
		if k==0: # 模型训练,只做一次
			if retrain:
				# 模型初始化
				model=deeplearning_methods[forecast_model]()
				# model=deeplearning_methods[forecast_model](**dp_parameters)
				# 模型训练
				model.fit(X_train,array(y_train))
