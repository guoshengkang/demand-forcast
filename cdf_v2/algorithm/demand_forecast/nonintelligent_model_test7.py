...
from integrated_timeseries_merge_model import *
from integrated_deeplearning_model import *

from get_data_from_db import *

fin_path = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'mape7.csv')
...
def nonintelligent_model(input_data_df=None,config_data_df=None,date_start=None,date_end=None,time2market=None,forecast_model='GBRT',
	...
	for index_no,id in enumerate(obj_ids): 
		...
		y_train=timeseries_merge(y_train,length_merge=7)
		y_test=timeseries_merge(y_test,length_merge=7)
		y_all=timeseries_merge(y,length_merge=7)
		...
		if forecast_model in regression_methods:
			...
			y_real,y_fit,y_predict,error_fit=integrated_timeseries_merge_model(df=df_tmp,forecast_model=forecast_model,evaluation=evaluation,length_merge=length_merge,period=period)
		...
		#############################################
		pre_mape=prediction_MAPE_array(y_test=array(y_test),y_predict=array(y_predict))
		pre_mape=list(pre_mape)
		new_line=str(id)+','+str(pre_mape)+'\n'
		print(new_line)
		fout.write(new_line)
		...
		# plt.plot(y_all,'o-',color='purple',label='real_data')
...
if __name__=='__main__':
	...
	json_result=nonintelligent_model(input_data_df,config_data_df,date_start=date_start,date_end=date_end,time2market=time2market,
			forecast_model='HWES',evaluation='MAPE',length_merge=7,period=4)
